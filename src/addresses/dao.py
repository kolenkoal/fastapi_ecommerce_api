...
from src.database import async_session_factory, superior_roles_id
...
class AddressDAO(BaseDAO):
    ...
    @classmethod
    async def _check_existing_address(cls, existing_address, user):
    ...
    @classmethod
    async def _user_find_all(cls, user):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def _superior_user_find_all(cls):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def add(cls, user: User, **data):
        async with async_session_factory() as session:
            ...
            get_user_address_added_query = select(
    ...
    @classmethod
    async def set_to_default(cls, address_id: UUID, user: User):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def _change_default_address(cls, address_id: UUID, user: User):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def find_by_id(cls, user: User, address_id: UUID):
        async with async_session_factory() as session:
            # Get all user ids, who use this address
    ...
    async def change_address(
        cls, address_id: UUID, user: User, address_data: SAddressOptional
    ):
        async with async_session_factory() as session:
            ...
            get_address_query = select(cls.model).where(
                cls.model.id == address_id
            )
    ...
    async def _update_to_existing_address(
        cls, user, address_id, existing_address
    ):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def _update_to_new_address(cls, address_data, address_id):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def _create_new_address(cls, new_address_data, address_id, user):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def _remove_user_address(cls, user, address_id):
        async with async_session_factory() as session:
            get_user_address_query = select(UserAddress).where(
                and_(
                    UserAddress.address_id == address_id,
    ...
    @classmethod
    async def _delete_certain_address(cls, user, address_id):
        async with async_session_factory() as session:
    ...
    @classmethod
    async def delete_address(cls, user, address_id: UUID):
        async with async_session_factory() as session:
            get_address_users_ids_query = (
                select(UserAddress.user_id)
                .select_from(UserAddress)
